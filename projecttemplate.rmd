...
ggplot(aes(x=LoanStatus, color=LoanStatus), data=subset(loans, !is.na(LoanStatus)))+
...
Look at what Charge-offs look like

```{r echo=FALSE, message=FALSE, warning=FALSE, ChargedoffFreqpoly}

ggplot(aes(x=LP_GrossPrincipalLoss), data=subset(loans, LoanStatus=='Chargedoff'))+
  geom_freqpoly()+
...
It appears Stated Monthly Income was not tracked very closely.
...
ggplot(aes(x=StatedMonthlyIncome), data=loans)+
  ...
  xlim(0,10)+
  ylim(0,300)
  ggtitle('Number of Loans by Stated Monthly Income')
...
ggplot(aes(x=ListingCategory..numeric.), data=subset(loans, LoanStatus=='Chargedoff'))+
  geom_histogram()+
...
Not surprisingly the number of charge-offs are greater with newly employed individuals

```{r echo=FALSE, message=FALSE, warning=FALSE, Chargeoff_EmploymentsStatusDuration}
ggplot(aes(x=EmploymentStatusDuration), data=subset(loans, LoanStatus=='Chargedoff'))+
  geom_histogram()+
...
ggplot(aes(x=DelinquenciesLast7Years), data=subset(loans, LoanStatus=='Chargedoff'))+
  geom_histogram()+
...
rates are high.  I thought the high charge-offs might
be due to the Great Recession given the period identitified in the summary below.
Unfortunately, there is not sufficient data points to tell us when the charge-offs
began or the dates loans were charged-off so although we can speculate the 
charge-offs were related to the Great Recession, we have no modeling to base 
that upon.
...
```
Given the above and number of variables, I did not yet need to create any additional variables 
but I did subset the loans portfolio quite often so I was only looking that portion of
loans which were charged-off.  Next I will attempt to evaluate the variables most closely
correlated with charge-offs.
...
```
Lack of significant correlation between LP_GrossPrincipalLoss and Percent Funded.  
...
# Wonder if credit scores are similar for borrowers w/out investmentFromFriends?

```


See if there is a visual correlation between Loan Status and Employment Duration
...
```{r echo=FALSE, message=FALSE, warning=FALSE, COR-LoanStatus_EmploymentDuration}

cor.test(loans$LP_GrossPrincipalLoss, loans$EmploymentStatusDuration)
```
```{r echo=FALSE, message=FALSE, warning=FALSE, LoanStatus_EmploymentDuration-point}
ggplot(aes(x=EmploymentStatusDuration, y=LP_GrossPrincipalLoss), data=subset(loans, LoanStatus=='Chargedoff'))+
  geom_point(na.rm = TRUE)+
  ggtitle('Gross Principal Loss vs Employment Status Duration for Charged-off Loans')
...
R1<-ggplot(aes(x=IsBorrowerHomeowner, y=LP_GrossPrincipalLoss), data=subset(loans, IsBorrowerHomeowner=='True'))+
  geom_jitter(alpha=1/20)+
  ggtitle('Gross Principal Loss vs Homeowners')

R2<-ggplot(aes(x=IsBorrowerHomeowner, y=LP_GrossPrincipalLoss), data=subset(loans, IsBorrowerHomeowner=='False'))+
  geom_jitter(alpha=1/20)+
...
```{r echo=FALSE, message=FALSE, warning=FALSE, LoanStatus_DelinquenciesPast7Years}
...
ggplot(aes(x=LP_GrossPrincipalLoss, y=ListingCategory..numeric.), data=subset(loans, LoanStatus=='Chargedoff'))+
...
Replot Above graph by ratio.  There certainly seems to be some telling information
based on the Listing Category.  Espeically 1-Debt Consolidation, 3-Business and 7-Other.
...
ggplot(aes(x=LP_GrossPrincipalLoss, y=CreditScoreRangeUpper), data=subset(loans, LoanStatus=='Chargedoff'))+
  geom_line(alpha=1/2)+
...
# there is a correlation between charge-offs and losses but it is the HIGHER credit scores which have 
# larger losses.  Perhaps because higher credit scores get higher loans.
...
I thought there would be a clear relationship between the percentage of the loan
funded and Charge-offs.  There was not.  Likewise, I believed that if a borrower
had a loan partially funded from a friend, then they would be less likely to 
default because their friend would lose money.  This relationship did not hold.
...
I was happy to find a negative correlation between employment status and Charge-
off such that the longer tenured employees had less losses.  The correlation was
-0.03622925 Not strongly correlated, but correlated nonetheless.
...
```{r echo=FALSE, message=FALSE, warning=FALSE, EmploymentStat-DebitConsolodation}
ggplot(aes(x=LP_GrossPrincipalLoss, 
           y=EmploymentStatusDuration), 
            data=subset(loans, ListingCategory..numeric.=='1' & LP_GrossPrincipalLoss>0))+
  geom_point(color='red')+
  stat_smooth(method='lm')+
  ggtitle('Employment Stat Duration in Mo vs Gross Principal Loss for Debt Consolodation Loan')
...
ggplot(aes(x=LP_GrossPrincipalLoss, 
           y=EmploymentStatusDuration), 
            data=subset(loans, ListingCategory..numeric.=='3' & LP_GrossPrincipalLoss>0))+
  geom_point(color='blue')+
  stat_smooth(method='lm')+
  ggtitle('Employment Stat Duration in Mo vs Gross Principal Loss for Business Loan')
...
Q1<-ggplot(aes(x=LP_GrossPrincipalLoss), data=subset(loans, DebtConsolodation>0))+
  geom_freqpoly()+
...
loans$ThreeLoanTypes<-loans$DebtConsolodation+loans$BusinessLoan+loans$OtherLoan
...
# IT sure appears that these 3 loan types were responsible for much of the losses
```
...
Each of the three loan types, however were correlated with Charge-offs individually.
Debt Consolodation had the strongest correlation at 0.551891.
Business was next at 0.3672464.
...
ggplot(aes(x=100*(LP_GrossPrincipalLoss/(sum(loans$LP_GrossPrincipalLoss))), 
  ...
  xlab('Ratio of Individual Loan Gross Principal Loss divided by total Gross Principal Loss')+
  ggtitle('Listing Category by Gross Principal Loss Ratio')
...
total Charged-off in a point plot by Listing Category.  As you can see the darkest
lines are for Listing Categories 0 (NA), 1(Business) and 7 (Other).  I included this
plot because it was one of the first plots which allowed me to identify a variable
which could be tightly correlated to Charge-offs.  

You can see from the darkness of teh line next to the 4 above listing categories 
that not only do they have the most data points but those data points tend to
be on the larger end of the x Axis as well.

I originaly had alpha = to 5% but in the remake of this, I deleted it and liked 
it better with the darker points.  I also made it clear that each point was a loan
which was charged off and each represented a % of the total charged-off
...
It was interesting too that I could get a feel for the lona portfolio 
...
in a column can be very difficult.  Please contact me at 
kenm@eurekapayments.com for further questions or comments.
